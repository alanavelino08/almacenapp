...
//     // Validación de campos de información de la parte
//     if (!requestData.customer_pn || !requestData.nickname || !requestData.project) {
//       setError('La parte no existe. Favor de agregar numero de parte!');
//       return;
//     }
...
//         <Grid container spacing={2}>
//           <Grid item xs={12} md={6}>
//             <TextField fullWidth label="Pedido" name="order" value={requestData.order} onChange={handleRequestChange} />
//           </Grid>
//           <Grid item xs={12} md={6}>
  ...
  const handleCreateShipment = async () => {
    ...
    try {
      ...
      setLastShipments(prev => {
        ...
        return [newShipment, ...previousShipments].slice(0, 5); // Mantener solo los 5 más recientes
      });
    } catch (err) {
      console.error('Error al crear embarque', err);
      setError('Hubo un error al crear el embarque.');
