...
const CreatePart = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // alert('Error:', error.response.data.detail || 'Error al crear parte');
            // console.error('Error creating part:', error);
            if (error.response && error.response.status === 400) {
            // Verifica si el error es por duplicado
            if (error.response.data.ikor_number) {
                alert(`Error: El número de parte "${formData.ikor_number}" ya existe.`);
            } else {
                alert('Error: ' + (error.response.data.detail || 'Datos inválidos'));
            }
        } else {
            alert('Error inesperado: ' + (error.message || 'Inténtalo de nuevo'));
        }
        console.error('Error creating part:', error);
        }
    };
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            <TableRow key={part.id}>
                                ...
                                <IconButton 
                                onClick={() => handleDeleteClick(part.id)} color="error">
                                    <Delete />
                                </IconButton>
                                </TableCell>
                            </TableRow>
