...
const ShipmentsList = ({ showAll }) => {
  ...
  //CODIGO ANTERIOR PARA RESTAURAR
  ...
  const fetchShipments = async (page = 1) => {
    try {
      ...
      if (isWarehouseUser) {
        ...
        params.append('taked_by', user.id);
      ...
      if (isWarehouseUser || !response.data.results) {
        setShipments(Array.isArray(response.data.results) ? response.data.results : response.data);
          ...
          <TableBody>
              {(isWarehouseUser ? processedShipments : (tabValue === 0 ? pendingShipments : processedShipments))
                .filter(shipment => 
                  ...
                  if (numA !== numB) {
                    return numB - numA; // primero por shipment_code desc
                  }
              ...
              <TableRow key={shipment.id}>
                <TableCell
                // onClick={() => handleClickOpenTransport(shipment)}
                onClick={() => {
                  if (isPlannerUser) {
                ...
                <TableCell>
                  ...
                  {/*Dialog para formulario de albaran */}
